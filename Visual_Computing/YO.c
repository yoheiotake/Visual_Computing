...
void display(void)
{
	...
	glTranslatef( -1.5, 2.0+3*dist, 100.0 );
	...
	glPushMatrix();
	glScalef(0.1,0.1,0.1);
	glTranslatef( -1.5, 2.0, 150.0 );
	glRotatef (theta, 1.0, 1.0, 0.0);
	glRotatef (-90, 1.0, 0.0, 0.0);
	drawArmShadingO(arm1);	/*  ロボットの腕No.0の描画 */
	glPopMatrix();
	
	//**********************************
	/*
	glTranslatef( 1.5, 2.0, 10.0 );
	glRotatef (theta, 1.0, 1.0, 0.0);//thetaが変化するので回転する
	...
	*/
	...
	glutWireTorus( 0.2, 1.0, 15,30 );//ドーナッツ glutSolidTorusにすると塗りつぶされる。速度も落ちる
	glPopMatrix();
...
void myInit (char *progname)
{
	...
	glMatrixMode(GL_PROJECTION);//投影変換設定
	glLoadIdentity();
	gluPerspective(60.0, aspect, 0.1, 40.0);//透視変換セット
	glMatrixMode(GL_MODELVIEW);//モデルビューモード設定
